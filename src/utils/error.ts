...
export function createError(
  ...
  originatingService: string = "com.epicgames.account.public",
  intent: string = "FortMP",
...
// https://github.com/simplyzetax/Aphrodite/blob/06ae5524e88bc1fe7f0652aba61f4a1f5a34f9b8/src/utils/error.ts
...
export class ApiError {
  ...
  variable(variables: string[]): this {
    ...
    for (const placeholderIndex of replacables) {
      ...
      if (variable) {
        this.response.errorMessage = this.response.errorMessage.replace(
          `{${placeholderIndex}}`,
          variable
        );
      }
  ...
  with(...messageVariables: string[]): this {
    this.response.messageVars =
      this.response.messageVars?.concat(messageVariables) || messageVariables;
    return this;
  }
  ...
  getMessage(): string {
    return (
      this.response.messageVars?.reduce(
...
/**
 * @description This is simplyzetax's error codes from Aphrodite. All credits to him.
 * @author simplyzetax
 * @see https://github.com/simplyzetax/Aphrodite/blob/06ae5524e88bc1fe7f0652aba61f4a1f5a34f9b8/src/utils/error.ts#L96
 */
export const FortMP = {
  proxy: {
    get fetchError() {
  ...
  gamecatalog: {
    get invalidParameter() {
      return new ApiError(
        "errors.com.epicgames.gamecatalog.invalidParameter",
    ...
    itemNotFound(offerId: string) {
      return new ApiError(
        "errors.com.epicgames.mcp.catalogOutOfDate",
        `Could not find catalog item ${offerId}`,
        28001,
        400,
        offerId
      );
    ...
    purchaseNotAllowed(
      ...
      return new ApiError(
        "errors.com.epicgames.gamecatalog.purchaseNotAllowed",
  ...
  matchmaking: {
    ...
    get player_banned_from_sub_game() {
      return new ApiError(
  ...
  friends: {
    ...
    get requestAlreadySent() {
      return new ApiError(
        ...
        "Friendship request has already been sent.",
        14014,
        409
      );
    },
    get friendshipExists() {
      return new ApiError(
        "errors.com.epicgames.friends.friendshipExists",
  ...
  internal: {
    ...
    get eosError() {
      return new ApiError(
        ...
        "Sorry an error occurred while communication with Epic Online Service Servers.",
        1001,
        500
      );
    },
