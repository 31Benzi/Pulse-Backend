...
app.post(
  ...
  async (c) => {
    ...
    if (
      ...
      !Array.isArray(body.itemFavStatus) ||
      body.itemIds.length !== body.itemFavStatus.length
    ) {
      return c.json({});
    }

    const itemIds = body.itemIds;
    const favStatuses = body.itemFavStatus;
    ...
    for (let i = 0; i < itemIds.length; i++) {
      ...
      const isFavorite = favStatuses[i];
      ...
      if (valueObj.favorite !== isFavorite) {
        valueObj.favorite = isFavorite;
